...
// Allow overriding uploads directory for deployments (e.g., Railway volumes)
...
const serveStatic = (req, res) => {
	...
	fs.stat(resolved, (err, stat) => {
		...
		const map = {
			...
			'.jpeg': 'image/jpeg', '.gif': 'image/gif', '.mp4': 'video/mp4', '.webm': 'video/webm', '.mov': 'video/quicktime'
		};
...
const routes = async (req, res) => {
    // Serve uploads from configured directory, regardless of location
    if (req.url.startsWith('/uploads/')) {
        const relPath = decodeURIComponent(req.url.replace(/^\/uploads\//, ''));
        const resolved = path.join(UPLOADS_DIR, relPath);
        ...
        fs.stat(resolved, (err, stat) => {
            ...
            const map = {
                '.html': 'text/html', '.css': 'text/css', '.js': 'application/javascript',
                '.json': 'application/json', '.png': 'image/png', '.jpg': 'image/jpeg',
                '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.mp4': 'video/mp4', '.webm': 'video/webm', '.mov': 'video/quicktime', '.m4v': 'video/x-m4v'
            };
            res.writeHead(200, { 'Content-Type': map[ext] || 'application/octet-stream', 'Access-Control-Allow-Origin': '*' });
...
const server = http.createServer((req, res) => {
	...
	if (req.url.startsWith('/uploads/')) {
		const file = decodeURIComponent(req.url.replace('/uploads/', ''));
		...
		fs.stat(resolved, (err, stat) => {
			...
			const ext = path.extname(resolved).toLowerCase();
			const map = { '.mp4': 'video/mp4', '.webm': 'video/webm', '.mov': 'video/quicktime', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg' };
			res.writeHead(200, { 'Content-Type': map[ext] || 'application/octet-stream' });
			fs.createReadStream(resolved).pipe(res);
		});
		return;
